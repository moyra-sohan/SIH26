...
export function AuthProvider({ children }) {
  ...
  const verifySession = useCallback(async () => {
    try {
      ...
      // If backend returns unauthorized or invalid session/cookie
      setUser(null);
  ...
  const login = async (identifier, password) => {
    ...
    const res = await fetch(`${API_BASE_URL}/api/auth/login`, {
      ...
      credentials: 'include', // Ensures cookies & session are received and stored
  ...
  const logout = async () => {
    try {
      ...
      await fetch(`${API_BASE_URL}/api/auth/logout`, {
        method: 'POST',
        headers,
        credentials: 'include', // Sends cookie to clear backend session & cookie
      });
    } catch (err) {
